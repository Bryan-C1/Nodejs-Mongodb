...
function confirmarCompra(total) {
  return new Promise((resolve) => {
    ...
    try {
        await validarUsuario("Brayan");
        const producto = await validarStock({ nombre: "Laptop", precio: 2000000, cantidad: 5, stock: 5 });
...
async function realizarCompra() {
  try {
    await validarUsuario("Brayan");
    const producto = await validarStock({ nombre: "Laptop", precio: 2000000, cantidad: 4, stock: 5 });
    const total = await calcularTotal(producto);
    const mensaje = await confirmarCompra(total);
    console.log(mensaje);
  } catch (error) {
